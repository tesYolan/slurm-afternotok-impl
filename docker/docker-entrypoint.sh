...
then
    ...
    # Run as root and let slurmctld drop privileges via SlurmUser
...
then
    ...
    # If container name looks like valid node names (d1-d10, n1-n5, w1-w2, p1, or legacy c1-c4)
    if [[ "$CONTAINER_NAME" =~ ^(c|d|n|w|p)[0-9]+$ ]]; then
